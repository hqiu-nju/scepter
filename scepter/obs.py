...
- Helper functions for angular separations and power conversion
...
class receiver_info():
    ...
    location : numpy.ndarray
        Observer location(s) on Earth as ``cysgp4.PyObserver`` objects.
    array_geometry : scepter.uvw.AntennaArrayGeometry or None
        Parsed array metadata when *pyobs* is an ASCII array-layout file.
    antenna_names : tuple of str or None
        Antenna names from the array-layout file when available.
    freq : astropy.Quantity
    ...
    ... )
    >>>
    >>> # Or load an interferometric array layout directly from CSV/text.
    >>> rx = receiver_info(
    ...     d_rx=13.5*u.m,
    ...     eta_a_rx=0.7,
    ...
    - Array-layout files are parsed by ``scepter.uvw.load_telescope_array_file``.
      Longitude and latitude are degrees, and altitudes default to metres.
    ...
    def __init__(
        ...
        bandwidth,
        tsys=20*u.K,
        array_altitude_unit: u.UnitBase | None = None,
    ):
        """
        ...
        pyobs : cysgp4.PyObserver, array of PyObserver, or str/path-like
            Observer object(s) containing location information (lon, lat, alt),
            or an ASCII telescope-array layout file. Array files may be CSV,
            tab-separated, or whitespace-delimited text accepted by
            ``scepter.uvw.load_telescope_array_file``. Headered files should
            contain longitude, latitude, and altitude columns using aliases such
            as ``lon_deg``, ``lat_deg``, and ``alt_m``. Headerless files are
            interpreted as ``lon_deg lat_deg alt`` with an optional fourth name
            column.
        freq : astropy.Quantity
            Center frequency of the receiver band (Hz)
        bandwidth : astropy.Quantity
        ...
        tsys : astropy.Quantity, optional
            System temperature of the receiver (K), default 20 K
        array_altitude_unit : astropy.units.UnitBase, optional
            Unit for the altitude column when *pyobs* is an array-layout file.
            Defaults to metres. Parsed altitudes are converted to kilometres
            before creating ``cysgp4.PyObserver`` objects.

        Raises
        ------
        ImportError
            If *pyobs* is an array-layout file and ``scepter.uvw`` cannot build
            the required Astropy or cysgp4 objects.
        FileNotFoundError
            If *pyobs* is an array-layout path that does not exist.
        ValueError
            If an array-layout file is empty or malformed.

        Notes
        -----
        When *pyobs* is a file path, ``array_geometry`` stores the parsed
        ``scepter.uvw.AntennaArrayGeometry`` object and ``antenna_names`` stores
        names in file order. Existing direct ``PyObserver`` inputs keep
        ``array_geometry`` and ``antenna_names`` as ``None``.
        """
        self.d_rx = d_rx
        self.eta_a_rx = eta_a_rx
        self.array_geometry = None
        self.antenna_names = None
        if isinstance(pyobs, (str, Path)):
            from .uvw import load_telescope_array_file

            self.array_geometry = load_telescope_array_file(
                pyobs,
                altitude_unit=array_altitude_unit,
            )
            self.antenna_names = self.array_geometry.antenna_names
            self.location = self.array_geometry.pyobservers
        else:
            self.location = np.atleast_1d(np.asarray(pyobs, dtype=object))
        self.freq = freq
...
class obs_sim():
    ...
    def create_baselines(self):
        ...
        See Also
        --------
        scepter.uvw.baseline_pairs : Underlying calculation
    ...
    def baselines_nearfield_delays(self,mode = 'tracking'):
        ...
        scepter.uvw.baseline_nearfield_delay : Underlying delay calculation
        scepter.uvw.geometric_delay_az_el : Far-field geometric delay
        sat_fringe : Use delays to calculate fringes
        """
        from . import uvw


        reference_observer = self.location.flatten()[0]
        ref_lon_rad = np.deg2rad(reference_observer.loc.lon)
        ref_lat_rad = np.deg2rad(reference_observer.loc.lat)
        l1 = self.topo_pos_dist[0][np.newaxis, :] * u.km
        if mode == 'tracking':
            pnt_az = (
                self.pnt_az[0]
                if np.shape(self.pnt_az) and np.shape(self.pnt_az)[0] == 1
                else self.pnt_az
            )
            pnt_el = (
                self.pnt_el[0]
                if np.shape(self.pnt_el) and np.shape(self.pnt_el)[0] == 1
                else self.pnt_el
            )
            tau1 = uvw.geometric_delay_az_el(
                self.bearings * u.m,
                pnt_az,
                pnt_el,
                ref_lon_rad,
            ...
            self.pnt_tau = tau1
            self.baseline_delays = uvw.baseline_nearfield_delay(
                l1,
                self.topo_pos_dist * u.km,
                tau=tau1,
            )
        elif mode == 'allsky':
            tau1 = uvw.geometric_delay_az_el(
